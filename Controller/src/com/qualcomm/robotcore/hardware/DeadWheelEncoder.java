...
public class DeadWheelEncoder implements DcMotorEx {
    ...
    // our empirically-determined encoder counts per inch for our actual robot don't match the theoretical counts per inch of the specs of the device.
    // rev encoder we're using.  Rather than having to change our real code in TeamCode, we allow the virtual robot to scale the encoder counts
    ...
    public synchronized int getCurrentPosition() {
        int result = (int)Math.round((actualPosition - encoderBasePosition) * scale);
     ...
     * @param deltaRadiansObject Incremental clockwise rotation of the dead wheel in radians
     * @return
     */
    public synchronized Object updateState(double milliseconds, Object deltaRadiansObject){
        double radians = (double)deltaRadiansObject;
...
//        if (radians != 0.0) {
//            System.out.printf("ts: " + milliseconds + " " + this.getDeviceName() + ": position " + actualPosition + " change " + positionChange + "\n");
//        }
